...
{
    struct fun_desc menu[] =  {{NULL, NULL},{"Load signatures", load_signatures},{"Print signatures", print_signatures},{"Detect viruses", Detect_viruses},{"Fix file", fix_file},{"Quit", quit},{NULL, NULL}};
...
void list_free(link *virus_list){
    while(virus_list != NULL){
        ...
        free(virus_list->vir);
        link* temp = virus_list->nextVirus;
        free(virus_list);
        virus_list = temp;
     }
}
...
void fix_file(){
    ...
    start = atoi(fgets(number, 10, stdin));
    printf("Enter signature size: ");
    size = atoi(fgets(number, 10, stdin));
    kill_virus(filename, start, size);
}

void kill_virus(char* fileName, int signitureOffset, int signitureSize){
  FILE* infectedfile = fopen(fileName,"r+");
  char replacement[signitureSize];
  int i;
  for(i = 0; i < signitureSize; i++)
     replacement[i] = 0x90;
  fseek(infectedfile, signitureOffset, SEEK_SET);
  fwrite(replacement, sizeof(char), signitureSize, infectedfile);
  fclose(infectedfile);
}
